...
if __name__ == '__main__':
    ...
    use_hcam = False
    if 'use_aam' in cfg and cfg['use_aam']:
        use_aam = True
    ...
    for iter_idx in range(start_iter, cfg.max_iterations + 1):
        ...
        if use_ali_ams:
            ...
            cls, loc, att_ft = parallel_net(images)
            loss = criterion(cls, loc, targets, att_ft)
            cls_loss, loc_loss, att_loss, total_loss = loss
        else:
            cls, loc = parallel_net(images)
            loss = criterion(cls, loc, targets)
            cls_loss, loc_loss, total_loss = loss
        ...
        if iter_idx % cfg.log_smooth_window == 0:
            ...
            if use_hcam:
                ...
                print('Loss conf: {:.4f} Loss loc: {:.4f} Loss_aam: {:.4f}'.format(cls_loss.data, loc_loss.data, att_loss.data))
            else:
